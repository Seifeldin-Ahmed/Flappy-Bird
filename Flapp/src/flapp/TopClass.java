...
public class TopClass implements ActionListener, KeyListener {
    ...
    private void buildFrame() {
        Image icon = Toolkit.getDefaultToolkit().getImage(this.getClass().getResource("blue_bird.png"));
    ...
    private JPanel createContentPane() {
        ...
        pgs = new PlayGameScreen(SCREEN_WIDTH, SCREEN_HEIGHT, false); //true --> we want pgs to be the splash screen
        topPanel.add(pgs);
     ...
     */
    private void fade_and_start() {
 Thread t = new Thread() {
            @Override
            public void run() {
                topPanel.remove(startGame);
                topPanel.remove(pgs); // removed because we want temp panel above pgs panel	
                topPanel.revalidate(); //this an instruction to tell the layout manager to reset based on the new component list
                ...
                topPanel.add(temp); // add temp panel then add pgs panel
                topPanel.add(pgs);
                topPanel.revalidate();
                topPanel.repaint();

                long currentTime = System.currentTimeMillis();
                while (temp.getBackground().getAlpha() != 255) { // while transperncy not equal 255
                ...
                //at this moment alpha=255

                // becase we want to make a new pgs to start the game, then add it but pgs will be above temp panel
                //and we need to make temp panel above the pgs panel
                topPanel.removeAll();
                topPanel.revalidate();
                topPanel.add(temp);
                topPanel.revalidate();
